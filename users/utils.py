...
def search_profiles(request):
    ...
    profiles_by_name_intro = Profile.objects.filter(Q(name__icontains=query) | Q(short_intro__icontains=query)).distinct().annotate(projects_number=Count('project')).order_by('-projects_number')
    if not profiles_by_name_intro:
        profiles_by_skills = Profile.objects.get_profiles_by_skills(query).annotate(projects_number=Count('project')).order_by('-projects_number')
